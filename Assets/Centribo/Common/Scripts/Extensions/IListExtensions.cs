...
namespace Centribo.Common.Extensions {
	public static class IListExtensions {
		...
		public static T TryGetElement<T>(this IList<T> list, int index) where T : class {
		...
		public static Queue<T> GetRandomQueue<T>(this IList<T> list, int length = -1) {
			list.Shuffle();
		...
		/// Adapted from: https://stackoverflow.com/a/2094260
		/// </summary>
		public static void Swap<T>(this IList<T> list, int firstIndex, int secondIndex) {
			if (list == null) return;
			if (firstIndex < 0 || firstIndex >= list.Count) return;
			if (secondIndex < 0 || secondIndex >= list.Count) return;
